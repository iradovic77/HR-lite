        ...
        return await (
            ...
            select new SettlementResponse
            {
                ...
                NameHr             = _db.Translations
                    .Where(t => t.EntityType == "codebook_settlement" && t.EntityId == s.Id && t.LanguageCode == Hr && t.FieldName == "Name")
                    .Select(t => t.Value).FirstOrDefault() ?? s.Code,
                NameEn             = _db.Translations
                    .Where(t => t.EntityType == "codebook_settlement" && t.EntityId == s.Id && t.LanguageCode == En && t.FieldName == "Name")
                    .Select(t => t.Value).FirstOrDefault(),
                MunicipalityNameHr = _db.Translations
                    .Where(t => t.EntityType == "codebook_municipality" && t.EntityId == s.MunicipalityId && t.LanguageCode == Hr && t.FieldName == "Name")
                    .Select(t => t.Value).FirstOrDefault(),
                CountyNameHr       = _db.Translations
                    .Where(t => t.EntityType == "codebook_county" && t.EntityId == municipality.CountyId && t.LanguageCode == Hr && t.FieldName == "Name")
                    .Select(t => t.Value).FirstOrDefault(),
                CountryNameHr      = _db.Translations
                    .Where(t => t.EntityType == "codebook_country" && t.EntityId == county.CountryId && t.LanguageCode == Hr && t.FieldName == "Name")
                    .Select(t => t.Value).FirstOrDefault()
            }
        ).ToListAsync();
    }
