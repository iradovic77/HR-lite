        ...
        return await _db.Countries
            ...
            .Select(c => new CountryResponse
            {
                ...
                NameHr        = _db.Translations
                    .Where(t => t.EntityType == "codebook_country" && t.EntityId == c.Id && t.LanguageCode == Hr && t.FieldName == "Name")
                    .Select(t => t.Value).FirstOrDefault() ?? c.Code,
                NameEn        = _db.Translations
                    .Where(t => t.EntityType == "codebook_country" && t.EntityId == c.Id && t.LanguageCode == En && t.FieldName == "Name")
                    .Select(t => t.Value).FirstOrDefault(),
                CitizenshipHr = _db.Translations
                    .Where(t => t.EntityType == "codebook_country" && t.EntityId == c.Id && t.LanguageCode == Hr && t.FieldName == "Citizenship")
                    .Select(t => t.Value).FirstOrDefault(),
                CitizenshipEn = _db.Translations
                    .Where(t => t.EntityType == "codebook_country" && t.EntityId == c.Id && t.LanguageCode == En && t.FieldName == "Citizenship")
                    .Select(t => t.Value).FirstOrDefault()
            })
            .FirstOrDefaultAsync();
    }
