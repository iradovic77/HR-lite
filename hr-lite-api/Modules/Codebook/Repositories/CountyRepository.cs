...
public class CountyRepository : ICountyRepository
{
    ...
    public CountyRepository(AppDbContext db) => _db = db;
        ...
        return await (
            ...
            select new CountyResponse
            {
                ...
                NameHr        = _db.Translations
                    .Where(t => t.EntityType == "codebook_county" && t.EntityId == c.Id && t.LanguageCode == Hr && t.FieldName == "Name")
                    .Select(t => t.Value).FirstOrDefault() ?? c.Code,
                NameEn        = _db.Translations
                    .Where(t => t.EntityType == "codebook_county" && t.EntityId == c.Id && t.LanguageCode == En && t.FieldName == "Name")
                    .Select(t => t.Value).FirstOrDefault(),
                CountryNameHr = _db.Translations
                    .Where(t => t.EntityType == "codebook_country" && t.EntityId == c.CountryId && t.LanguageCode == Hr && t.FieldName == "Name")
                    .Select(t => t.Value).FirstOrDefault()
            }
        ).ToListAsync();
    }
